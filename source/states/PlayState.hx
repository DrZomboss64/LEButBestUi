...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(!yoWaitThisIsCharter)
		{
			...
			*/
			...
			songName = new FlxSprite(-20,0).loadGraphic(Paths.image('SongBar'));
			songName.cameras = [camHUD];
			//songName.alpha = 0;
			songName.x -= songName.width;
			songName.y = FlxG.height - 100 - songName.height;
			add(songName);
			...
			popupText = new FlxText(songName.x, songName.y + 20, songName.width, popText, 20);
	...
	**/
	public function fixedUpdate()
	{
		...
		#if linc_luajit
		setLuaVar("timeBar", Conductor.songPosition);
	...
	override public function update(elapsed:Float)
	{
		...
		scoreTxt.text = (
			(utilities.Options.getData("notesperSecond") == true ? "NPS: " + nps + "(" + maxNPS + ") // " : "") +
			"Score: " + songScore + " // " +
			"Misses: " + misses + " // " +
			"Accuracy: " + accuracy + "% // " +
			ratingStr
	...
	private function popUpScore(strumtime:Float, noteData:Int, ?setNoteDiff:Float):Void
	{
		...
		if(utilities.Options.getData("displayMs"))
		{
			...
			accuracyText.font = Paths.font("Koda135759-vmm2O.ttf");
