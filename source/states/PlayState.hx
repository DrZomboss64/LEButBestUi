...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
        ...
        if(utilities.Options.getData("missSounds"))
        {
            for(i in 0...2)
            {
                var sound = FlxG.sound.load(Paths.sound('missnote' + Std.string((i + 1))), 0.2);
		...
		if (!yoWaitThisIsCharter)
		{
			...
			switch (funnyTimeBarStyle.toLowerCase())
			{
				default: // includes 'leather engine'
					...
					if (utilities.Options.getData("barColors"))
						...
						timeBar.createFilledBar(FlxColor.BLACK, FlxColor.CYAN);					timeBar.pixelPerfectPosition = true;
				...
				case "psych engine":
					...
					if (utilities.Options.getData("barColors"))
						...
						timeBar.createFilledBar(FlxColor.BLACK, FlxColor.WHITE);					timeBar.pixelPerfectPosition = true;
				...
				case "new kade engine":// test this shit lol
					...
					Std.int(timeBarBG.height + 6), this, 'time', 0, songLength);
					timeBar.alpha = 0;
					timeBar.scrollFactor.set();
					timeBar.createGradientBar([FlxColor.BLACK], [boyfriend.barColor, dad.barColor]);
					timeBar.numDivisions = 800;
					add(timeBar);

					bar = new FlxSprite(timeBar.x, timeBar.y).makeGraphic(Math.floor(timeBar.width), Math.floor(timeBar.height), FlxColor.TRANSPARENT);
					bar.alpha = 0;
					add(bar);
			...
			{
				ratingText = new FlxText(10, 0, 0,"bruh", 20);
				ratingText.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				ratingText.screenCenter(Y);
				ratingText.borderSize = 2;
				ratingText.borderQuality = 2;
	...
	override public function update(elapsed:Float)
	{
		...
		{
			if (FlxG.keys.justPressed.SEVEN && !switchedStates && !inCutscene && !SONG.disableDebugButtons)
			{
				#if linc_luajit
				if (executeModchart && luaModchart != null)
				{
	...
	{
		scoreTxt.text = ((utilities.Options.getData("notesperSecond") == true ? "NPS: " + nps + " (" + maxNPS + ") // " : "") + "Score: " + songScore + " // " + "Misses: " + misses +  " // Health:" + Math.floor(health * 500) / 10 + "%" + " // " + "Accuracy: " + accuracy + "% // " + ratingStr);
		...
		if (generatedMusic && startedCountdown)
		{
				...
				if (characterPlayingAs == 0)
				{
					if (boyfriend.otherCharacters == null)
					{
						if (boyfriend.animation.curAnim != null)
							if (boyfriend.holdTimer > Conductor.stepCrochet * 4 * 0.0011  && !heldArray.contains(true))
			...
			else
			{
				...
				if (characterPlayingAs == 0)
				{
					...
					{
						if (boyfriend.animation.curAnim != null)
							if (boyfriend.holdTimer > Conductor.stepCrochet * 4 * 0.0011)
								if (boyfriend.animation.curAnim.name.startsWith('sing')
									&& !boyfriend.animation.curAnim.name.endsWith('miss'))
									boyfriend.dance();
					}
		...
		if (!note.wasGoodHit)
		{
			if (note.shouldHit && !note.isSustainNote)
			{
                ...
                if(combo%10 + 1 == 1){
					...
					if(!utilities.Options.getData("comboFlash")){
					...
					if(!utilities.Options.getData("comboSound")){
	...
	//dogshit
	/*function changeMania(funnySpeed:Int)// stolen from tposejank lol
	{
		...
		for (i in 0...playerStrums.members.length) {
			...
			oldStrum.setGraphicSize(Std.int(oldStrum.width));
		...
		for (i in 0...enemyStrums.members.length) {
			var oldStrum:FlxSprite = enemyStrums.members[i].clone();
			oldStrum.x = enemyStrums.members[i].x;
			oldStrum.y = enemyStrums.members[i].y;
			oldStrum.alpha = enemyStrums.members[i].alpha;
			oldStrum.scrollFactor.set();
			oldStrum.cameras = [camHUD];
			oldStrum.setGraphicSize(Std.int(oldStrum.width));
			oldStrum.updateHitbox();
	...
	public function processEvent(event:Array<Dynamic>)
	{
		switch (event[0].toLowerCase())
		{
			...
			case "hey!":
				...
				var time:Float = Std.parseFloat(event[3]);
